...
# c_: number of nonces guessed for one more bit
...
def experiment(qlen_, l_, d_, c_, beta_):
	...
	l = l_
	d = d_ 
	block_B = block_matrix([ [identity_matrix(c) * 2^(l + 2) * q , matrix(ZZ, c, d - c, [0]* c* (d-c) ) ] , [matrix(ZZ, d - c, c , [0]* c* (d-c) ) ,identity_matrix(d - c) * 2^(l + 1) * q ] ])


	block_C = matrix(ZZ, d, 2, [0]* 2* d )
	vector_t = matrix(ZZ, 1, d,  [ 2^(l + 1) * ZZ.random_element(1, q) for _ in range(d)])
	vector_v = alpha * vector_t - matrix(ZZ, 1, d, [ 2^(l + 1) * ZZ.random_element(1, floor(q/2^l)) if _ < c  else 2^(l + 2) * ZZ.random_element(1, floor(q/2^(l+1))) for _ in range(d)]) + matrix(ZZ, 1, d, d * [q])
...
success_cnt = 0
for i in range(0, 10):	
	if experiment(160, 3, 80, 10, 20) == True:
